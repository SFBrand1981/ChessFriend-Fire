...
module.exports = function (window) {
	...
	function savePGNs(gameArray, nodeArray, nameArray) {
		...
		for (var len = gameArray.length, i = id - len + 1; i <= id; i++) {
		    
		    var millions = Math.floor(i/1000000) * 1000000
		    var tenThousands = Math.floor( (i - millions)/10000) * 10000
		    var hundrets = Math.floor( (i - millions - tenThousands)/100) * 100
		    
		    var newPath = millions + '/' + tenThousands + '/' + hundrets
		    
		    if (newPath in addedPaths) {
		        // return id
		    } else {
		        addedPaths.push(newPath)
		        promises.push(ph.createPathFromNumber(i))
		    }
		}
    ...
    function displayFEN() {
	boardState.displayFEN("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1", false)
    }
	...
	db.db.nodes.get({game_id: parseInt(game_id)}, function (entry) {
		...
		var pgn_file = ph.pathFromNumber(game_id)
	...
	db.db.games.update(parseInt(game_id), {
	    ...
	    return db.db.players.put({
		name: data.white.toLowerCase(),
		fullName: data.white
	    })
	}).then(function() {
	    return db.db.players.put({
		name: data.black.toLowerCase(),
		fullName: data.black
	    })
	}).then(function() {
	    ...
	    return db.db.tags.bulkPut(taglist)
	...
	tags.addEventListener('input', function() {
	    var val = this.value.toLowerCase()
	    return db.db.tags.where('name')
		.startsWith(val)
		.limit(limit)
		.toArray().then(function(arr) {
		    
		    ahTags.list = arr.map( a => a.fullName)
		    ahTags.evaluate()
		})
	})
	...
	menu.append(new nw.MenuItem({
	    label: 'Copy FEN',
	    click: function(){
		clipboard.set(boardState.getFEN(), 'text')
	    }
	}))
