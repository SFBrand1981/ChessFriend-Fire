...
module.exports = function (window) {
	...
	var FEN_inp = window.document.getElementById("FEN_inp")
	var stm_inp = window.document.getElementById("stm_inp")
	var mvNr_inp = window.document.getElementById("mvNr_inp")

	
	FEN_inp.addEventListener('change', function(evt) {

	    var fen = FEN_inp.value
	    var stm = fen.split(' ')[1]
	    var mvNr = parseInt(fen.split(' ')[5])
	    
	    bh.validateFEN(fen, stm, mvNr).then(function(valid) {
		bh.setupBoardFromFEN(valid.FEN)
		stm_inp.value = stm
		mvNr_inp.value = mvNr
		selectAddNew()
	    }).catch(function(e) {
	...
	searchBtn.addEventListener('click', function (evt) {
	    ...
	    var fen = bh.getFEN()
	    var stm = stm_inp.value
	    var mvNr = mvNr_inp.value

	    if (selectedAction == 'search') {

		// search position
		searchPosition(fen, stm, mvNr)
    ...
    function searchBoardPosition() {

	var fen = bh.getFEN()
	var stm = fen.split(' ')[1]
	var mvNr = fen.split(' ')[5]

	searchPosition(fen.split(' ')[0], stm, mvNr)
    }
    

    function searchPosition(fen, stm, mvNr) {
    ...
    module.searchBoardPosition = searchBoardPosition
