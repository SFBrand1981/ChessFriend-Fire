...
module.exports = function (window) {
    ...
    function triggerGameSavedEvent() {
	...
	window.document.addEventListener("keydown", window.document.listenForAlphanumeric = function (evt) {
	    ...
	    if (evt.key === 's') {
		evt.preventDefault()
		commandBuffer.push('s')
	    }

	    if (commandBuffer[0] == 'x' && commandBuffer[1] == 'c') {
		commandBuffer = []
		insertComment('comment')
	    }

	    if (commandBuffer[0] == 'x' && commandBuffer[1] == 's') {
		commandBuffer = []
		triggerGameSavedEvent()
	    }
	...
	displayNotation()
	currentBoard = selectMove(currentBoard.curNodeIndx)
	sessionStorage.setItem(currentBoard.id, JSON.stringify(currentBoard))
	triggerGameEditedEvent(currentBoard.id)		
    }
    ...
    function doInsertNullMove(board) {

	var newBoard = bh.copyBoard(board)
	
	// test if move is new move
	var fen = ph.nullMove(currentBoard.FEN)
	var children = newBoard.nodes[newBoard.curNodeIndx]['children']
	...
	currentBoard = doInsertNullMove(currentBoard)
    }
    ...
    module.setupBoardFromFEN = setupBoardFromFEN
    module.insertNullMove = insertNullMove
